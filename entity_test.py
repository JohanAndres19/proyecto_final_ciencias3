...
def main(debug=False):
    ...
    if not os.path.exists(dot_folder):
        os.mkdir(dot_folder)
    metamodel_export(entity_mm, join(dot_folder, 'calculadora_meta.dot'))

    # Build Person model from person.ent file
    person_model = entity_mm.model_from_file(join(this_folder, 'calculadora.en'))

    # Export to .dot file for visualization
    model_export(person_model, join(dot_folder, 'calculadora.dot'))
